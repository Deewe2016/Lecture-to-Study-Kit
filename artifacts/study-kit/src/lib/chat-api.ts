...
export function subscribeToMessages(onMessage: (message: DbMessage) => void, onStatus?: (status: string) => void) {
  ...
  const join = () => send('phx_join', { config: { broadcast: { ack: false, self: false }, presence: { enabled: false }, postgres_changes: [{ event: 'INSERT', schema: 'public', table: 'messages' }], private: false }, access_token: getAccessToken() || token });
  ...
  socket.addEventListener('message', (event) => { let payload: any; try { payload = JSON.parse(event.data); } catch { return; } if (payload.event === 'phx_reply' && payload.topic === 'realtime:chat' && payload.payload?.status === 'ok') { joined = true; onStatus?.('SUBSCRIBED'); } else if (payload.event === 'postgres_changes') { const record = payload.payload?.data?.record || payload.payload?.record; if (record?.id && record?.sender_id && record?.text) onMessage(record as DbMessage); } else if (payload.event === 'phx_error' || payload.event === 'phx_close') { joined = false; onStatus?.('RECONNECTING'); reconnect(); } });
  ...
  const refreshTimer = window.setInterval(() => { const nextToken = getAccessToken(); if (joined && nextToken) send('access_token', { access_token: nextToken }); }, 60000);
